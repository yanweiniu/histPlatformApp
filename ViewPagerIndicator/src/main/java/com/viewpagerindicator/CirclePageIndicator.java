...
import com.viewpagerindicator.R.bool;
import com.viewpagerindicator.R.color;
import com.viewpagerindicator.R.dimen;
import com.viewpagerindicator.R.integer;
...
public class CirclePageIndicator extends View implements PageIndicator {
        ...
        if(this.mViewPager != null) {
            ...
            if(count != 0 && count <= 100) {
                if(this.mCurrentPage >= count) {
                    ...
                    if(!this.mSnap && this.mCurrentPage != count - 1) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if(super.onTouchEvent(ev)) {
            ...
            if(this.mViewPager != null && pageCount != 0 && pageCount <= 100) {
                ...
                switch(action) {
                    ...
                    case 3:
                        if(!this.mIsDragging) {
                            pointerIndex = this.getPageCount();
    ...
    public void setRecycleViewPager(ViewPager view, int pageCount) {
        this.setViewPager(view);
        this.mRecycleRealCount = pageCount;
        if(pageCount > 0) {
            int position = 50 - 50 % pageCount;
            this.setCurrentItem(position);
        }

    }

    public void setRecyclePageCount(int pageCount) {
        this.mRecycleRealCount = pageCount;
        this.requestLayout();
        this.invalidate();
    }
    ...
    public int getPageCount() {
        if(this.mRecycleRealCount > 0) {
            return this.mRecycleRealCount;
        } else {
            int pageCount = this.mViewPager.getAdapter().getCount();
            return pageCount > 1000?0:pageCount;
        }
    }

    private int getPosition(int position) {
        return this.mRecycleRealCount > 0?position % this.mRecycleRealCount:position;
    }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        this.mCurrentPage = this.getPosition(position);
        this.mPageOffset = positionOffset;
        this.invalidate();
        if(this.mListener != null) {
            this.mListener.onPageScrolled(this.getPosition(position), positionOffset, positionOffsetPixels);
        }
    ...
    public void onPageSelected(int position) {
        if(this.mSnap || this.mScrollState == 0) {
            this.mCurrentPage = this.getPosition(position);
            this.mSnapPage = this.getPosition(position);
            this.invalidate();
        }

        if(this.mListener != null) {
            this.mListener.onPageSelected(this.getPosition(position));
        }
