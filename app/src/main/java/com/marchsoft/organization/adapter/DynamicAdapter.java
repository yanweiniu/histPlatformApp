...
public class DynamicAdapter extends BaseAdapter{
    ...
    private int id;
    public DynamicAdapter(Activity context, List<Dynamic> mList){
        this.mList = mList;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.supervisor.setText(dynamic.getStudentName());
                    ...
                    public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                        try {
                            if (response.getInt("msg_code") == Constant.CODE_SUCCESS) {
                                ...
                                if (status == 0) {
                                    dynamic.setRegistering(dynamic.getRegistering()+1);
                                    holder.number.setText((dynamic.getRegistering())+"");
                                    holder.joinUs.setText("取消报名");
                                    holder.joinUs.setTextColor(mContext.getResources().getColor(R.color.main_text_disable_textcolor));
                                    holder.joinUs.setEnabled(false);
                                } else {
