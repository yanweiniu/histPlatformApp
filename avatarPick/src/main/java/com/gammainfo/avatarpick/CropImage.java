...
public class CropImage {
	...
	public static final File FILE_LOCAL = new File(FILE_SDCARD, "jianzhi");
	...
	private int mImageWidth;//保存图片时的宽
	 
	public CropImage(Context context, CropImageView imageView, Handler handler, int rectRatioX, int rectRatioY, int imageWidth) {
		mContext = context;
		mImageView = imageView;
		mImageView.getRatio(rectRatioX, rectRatioY);
		mRectRatioX = rectRatioX;
		mRectRatioY = rectRatioY;
	 ...
	 * @return
	 */
	public Bitmap cropAndSave(String path) {
		// final Bitmap bmp = onSaveClicked(BitmapFactory.decodeFile(path));
	...
	private Bitmap onSaveClicked(Bitmap bm) {
		...
		int width = mImageWidth;// dr.width(); // modify by yc
		int height = mImageWidth * mRectRatioY / mRectRatioX;// dr.width();
		Bitmap croppedImage = Bitmap.createBitmap(width, height,
	...
	public String saveToLocal(Bitmap bm) {
		String path = FILE_LOCAL + "image_" + System.currentTimeMillis()
				+ ".png";
		try {
	...
	Runnable mRunFaceDetection = new Runnable() {
		...
		private void makeDefault() {
			...
			int cropWidth = Math.min(width, height) * mRectRatioX / 5;
			int cropHeight = Math.min(width, height) * mRectRatioY / 5;
